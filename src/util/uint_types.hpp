// copied from: https://github.com/kurpicz/dsss/blob/master/dsss/util/uint_types.hpp
...
public:
    ...
    uint64_t ull() const { return ((uint64_t)high_) << low_bits | (uint64_t)low_; }

    //! implicit cast to an unsigned long long
    operator uint64_t() const { return ull(); }

    //! return the number as a uint64_t
    uint64_t u64() const { return ((uint64_t)high_) << low_bits | (uint64_t)low_; }

    //! prefix increment operator (directly manipulates the integer parts)
    UIntPair& operator++() {
        if (DSSS_UNLIKELY(low_ == low_max()))
            ++high_, low_ = 0;
        else
            ++low_;
        return *this;
    ...
    UIntPair operator+(const UIntPair& b) const {
        uint64_t add = low_ + uint64_t(b.low_);
        return UIntPair((Low)(add & low_max()),
    ...
    UIntPair operator-(const UIntPair& b) const {
        uint64_t sub = low_ - uint64_t(b.low_);
        return UIntPair((Low)(sub & low_max()),
    ...
    friend std::ostream& operator<<(std::ostream& os, const UIntPair& a) { return os << a.ull(); }

    //! return an UIntPair instance containing the smallest value possible
    static UIntPair min() {
        return UIntPair(std::numeric_limits<Low>::min(), std::numeric_limits<High>::min());
    }

    //! return an UIntPair instance containing the largest value possible
    static UIntPair max() {
        return UIntPair(std::numeric_limits<Low>::max(), std::numeric_limits<High>::max());
    }
...
class numeric_limits<dsss::UIntPair<HighType> > {
public:
    using UIntPair = dsss::UIntPair<HighType>;
