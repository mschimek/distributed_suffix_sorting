...
class PDC3 {
    ...
    struct MergeSamples {
        ...
        std::ostream& operator<<(std::ostream& os) {
            os << "((" << chars[0] << " " << chars[1] << ") (" << ranks[0] << " " << ranks[1]
               << ") " << index << ")";
            return os;
        }
...
public:
    ...
    std::vector<MergeSamples> construct_merge_samples(std::vector<char_type>& local_string,
        ...
        for (uint64_t i = 0; i < chars_at_proc; i++) {
            ...
            uint64_t local_index = i;
            uint64_t global_index = local_index + chars_before;
            uint64_t mod = global_index % 3;

            KASSERT(local_index + 1 < local_string.size());
            KASSERT(pos + rank1_offset[mod] < local_ranks.size());
            KASSERT(pos + rank2_offset[mod] < local_ranks.size());

            char_type char1 = local_string[local_index];
            char_type char2 = local_string[local_index + 1];
            index_type rank1 = local_ranks[pos + rank1_offset[mod]].rank;
            index_type rank2 = local_ranks[pos + rank2_offset[mod]].rank;
    ...
    void handle_recursive_call(std::vector<RankIndex>& local_ranks,
        ...
        std::vector<index_type> SA = rec_pdc3.call_pdc3(recursive_string);
        recursion_depth--;
    ...
    std::vector<index_type> call_pdc3(std::vector<char_type>& local_string) {
        timer.start("pdc3");
        ...
        uint64_t total_chars = std::accumulate(chars_at_proc.begin(), chars_at_proc.end(), 0) - 3;
    ...
    void report_memory(bool print_history = false) {
        ...
        uint64_t local_string_bytes = stats.local_string_sizes.front() * sizeof(int);
